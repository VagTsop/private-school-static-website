...
$(document).ready(function() {
 ...
 $("form").submit(function(e){
      ...
      $(".data-table tbody").append("<tr data-title='"+title+"'data-stream='"+stream+"'data-type='"+type+"'data-firstName='"+firstName+"'data-lastName='"+lastName+"'><td>"+title+"</td><td>"+stream+"</td><td>"+type+"</td><td>"+firstName+"</td><td>"+lastName+"</td><td><button class='btn btn-info btn-xs btn-edit'>Edit</button><button class='btn btn-danger btn-xs btn-delete'>Delete</button></td></tr>");
    ...
    $("body").on("click", ".btn-edit", function(){
     ...
     $(this).parents("tr").find("td:eq(2)").html('<input name="edit_type" value="'+type+'">');
     $(this).parents("tr").find("td:eq(3)").html('<input name="edit_firstName" value="'+firstName+'">');
     $(this).parents("tr").find("td:eq(4)").html('<input name="edit_lastName" value="'+lastName+'">');
    ...
    $("body").on("click", ".btn-cancel", function(){
      ...
      $(this).parents("tr").find("td:eq(3)").text(firstName);
      $(this).parents("tr").find("td:eq(4)").text(lastName);
    ...
    $(document).ready(function(){
     $('#title, #stream, #type, #firstName,#lastName').on("cut copy paste",function(e) {
      e.preventDefault();
    });
   });
